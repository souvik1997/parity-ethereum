...
impl ChainSync {
	...
	pub fn new<BC: ClientBackend>(
		...
		let mut sync = ChainSync {
			...
			new_blocks: BlockDownloader::new(BlockSet::NewBlocks, &chain_info.best_block_hash, chain_info.best_block_number, BC::need_proofs()),
			old_blocks: None,
	...
	pub fn update_targets<BC: ClientBackend>(&mut self, chain: &BlockChainClient<StateBackend = BC>) {
		...
		self.new_blocks = BlockDownloader::new(BlockSet::NewBlocks, &chain.best_block_hash, chain.best_block_number, BC::need_proofs());
		...
		if self.download_old_blocks {
			if let (Some(ancient_block_hash), Some(ancient_block_number)) = (chain.ancient_block_hash, chain.ancient_block_number) {
				...
				let mut downloader = BlockDownloader::new(BlockSet::OldBlocks, &ancient_block_hash, ancient_block_number, BC::need_proofs());
